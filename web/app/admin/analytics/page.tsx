...
export default function AdminAnalyticsPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <h1 style={{ fontFamily: 'var(--font-display)', fontSize: '1.6rem', color: 'var(--secondary)', marginBottom: '1.75rem' }}>Analytics</h1>
      ...
      {loading ? (
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
            {[
              { label: 'All Time', value: totalAll, icon: <Globe size={16} /> },
              { label: 'Today', value: totalToday, icon: <Eye size={16} /> },
              { label: 'Last 7 Days', value: totalWeek, icon: <TrendingUp size={16} /> },
              { label: 'Last 30 Days', value: totalMonth, icon: <BarChart2 size={16} /> },
            ].map(({ label, value, icon }) => (
              <div key={label} style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.25rem' }}>
                ...
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '2.25rem', color: 'var(--text)', lineHeight: 1 }}>{value.toLocaleString()}</div>
              </div>
            ))}
          </div>

          {/* Daily chart */}
          <div style={{ background: 'white', border: '1px solid var(--border)', borderRadius: '1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          <p style={{ marginTop: '1.25rem', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
            Views are tracked via the <code>page_views</code> Supabase table. Admin pages are excluded.
          </p>
        </>
      )}
    </div>
        ...
        {data.map((d, i) => {
          ...
          return (
            <g key={i}>
              ...
              {d.value > 0 && (
                <text x={x + barW / 2} y={y - 5} textAnchor="middle" fontSize={9} fill="var(--secondary)" fontWeight="600">
                  {d.value}
                </text>
              )}
              {/* Only show label every other day on small screens */}
              {i % 2 === 0 && (
                <text x={x + barW / 2} y={H + 20} textAnchor="middle" fontSize={8.5} fill="var(--text-muted)">
                  {d.label}
                </text>
              )}
            </g>
